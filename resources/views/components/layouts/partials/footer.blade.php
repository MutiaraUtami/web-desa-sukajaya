@php
    // Ambil data profil desa menggunakan method get() yang sudah Anda definisikan di Model
    $profil = \App\Models\ProfilDesa::get();
    
    // Siapkan data dengan fallback (nilai bawaan) jika kolom di database masih kosong
    $alamat = $profil->alamat_kantor ?? 'Jl. Raya Sukatani, Desa Sukajaya, Kecamatan Sukatani, Kabupaten Purwakarta, Jawa Barat 41167.';
    $email = $profil->email ?? 'sukajaya-purwakarta@desa.id';
    $telepon = $profil->telepon ?? '081973492210';
    
    // Format nomor telepon untuk link wa.me (Bersihkan spasi/simbol, lalu ubah awalan 0 jadi 62)
    $waLink = preg_replace('/[^0-9]/', '', $telepon);
    if (str_starts_with($waLink, '0')) {
...
<footer class="bg-[#0e2206] text-gray-300 pt-16 pb-8 border-t-4 border-[#fac81b]">
    <div class="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
        ...
        <div class="space-y-4">
            ...
            <ul class="space-y-3 text-sm text-zinc-300">
                ...
                <li class="flex items-center gap-3">
                    ...
                    <a href="mailto:{{ $email }}" class="hover:text-[#fac81b] transition-colors duration-200">{{ $email }}</a>
                </li>
                
                <!-- WhatsApp / Telepon (Dinamis) -->
                <li class="flex items-center gap-3">
                    ...
                    <a href="https://wa.me/{{ $waLink }}" target="_blank" rel="noopener noreferrer" class="hover:text-[#fac81b] transition-colors duration-200">{{ $telepon }}</a>
                </li>
                
                <!-- Instagram (Statis - karena belum ada di database) -->
                <li class="flex items-center gap-3">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-2 text-sm">
                ...
                <a href="{{ route('struktur-organisasi') }}" class="hover:text-[#fac81b] transition-colors duration-200">Struktur Pemerintahan</a>
    ...
    <div class="max-w-6xl mx-auto px-4 pt-8 border-t border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-zinc-300">
        ...
        <p>Website Resmi Desa Sukajaya</p>
    </div>
</footer>